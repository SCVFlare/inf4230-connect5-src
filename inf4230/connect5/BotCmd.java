 ...
 * @author Eric Beaudry
 */
public class BotCmd implements Joueur {

    public BotCmd(String command) throws IOException {
        ...
        }while(!ligne.equalsIgnoreCase("PRET"));
    ...
    public static void main(String args[]) throws Exception{
        BotCmd j = new BotCmd("/home/user/inf4230/tp1/tp1");
        Grille g = new Grille(10, 10);
        GrilleVerificateur verif = new GrilleVerificateur();
        int next=0;
        while(true){
            if(g.getNbCasesLibres()==0){
                System.out.println("Partie nulle");
                break;
            }
            Position coup = j.getAction(g, 2000);
            if(g.get(coup)!=0){
                System.out.println("Placement illégal!");
                break;
            }
            g.set(coup, next+1);
            int winner = verif.getGagnant(g);
            if(winner>0){
